				...
				<li  class="item_wrap" @click="goToDetail(index)">
					...
					<div class=" content_wrap" v-for="(ite,ind) in item.order_goods" :key="ind">
						<div class="col_between border" >
							<div class="content_up">
								<img :src="ite.goods_list_img" mode="widthFix"/>
								<span class="content_center">{{ite.name}}</span>
							...
							<div class="content_down" v-if="ite.coupon_price">
								<span class="coupon" >优惠金额：￥{{ite.coupon_price}}</span>
								<!-- <span class="sencod_txt">应付金额:
									<span class="money">￥</span>
									<span class="money_number">{{ite.pay_price?parseInt(ite.price)-parseInt(ite.pay_price):}}</span>
					...
					<div class="bottom_wrap" >
						...
						<div class="btn_wrap" v-if="item.order_status>0">
							...
							<span class="pay" v-if="item.order_status == 1&&parseInt(item.price) > 0" @click.stop="moneyPay(item.id,item.price)">支付</span>
...
<script>
	import { BASE_IMAGE_URL,order_list,cancel_order,Refund_mallOrder } from '@/utils/api'
	...
	export default {
		...
		methods: {
			...
			async getList(){
				...
				let res = await order_list(params);
			...
			goToDetail(index, type){
				if(type == 1){
					uni.navigateTo({
						url: '/pages/mall_order_detail/mall_order_detail?type=1&orderID='+this.list[index].id
					});
				}else{
					uni.navigateTo({
						url: '/pages/mall_order_detail/mall_order_detail?orderID='+this.list[index].id
					});
				}
...
.btn_wrap .cancle{color: #666; font-size: 24upx; padding: 6upx 26upx; background-color: #fff;border-radius: 8upx;text-align: center;border: 1upx solid #999;margin: 0upx 10upx;}
