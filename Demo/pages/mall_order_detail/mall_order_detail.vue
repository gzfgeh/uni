		...
		<ul class="express-list">
			...
			<li  class="item_wrap">
					...
					<div class="col_between border" >
						<div class="content_up">
							...
							<div class="content_end">
								...
								<span class="money_number">￥{{ite.price}}</span>
							</div>
						</div>
						
						<div class="content_down" v-if="ite.coupon_price">
							<span class="coupon" >优惠金额:<span style="color: #333333;">￥{{ite.coupon_price}}</span></span>
						</div>
					</div>
				</div>
				<div class="tatol_wrap">
					...
					<div>
						<span class="freight_wrap">运费:￥{{item.freight}}</span>
						<span>共计金额:<span class="money_number">￥{{tatol}}</span> </span>
					</div>
...
<script>
	import { BASE_IMAGE_URL,order_detail,cancel_order,Refund_mallOrder,addressManage,getMallExpressInfoByorder } from '@/utils/api'
	...
	export default {
		...
		methods: {
			...
			async order_detail(){
				let res = await order_detail({order_id: this.orderID});
				if(res.status == 1){
