			...
			<block  v-for="(item,index) in list" :key="index" >
				<li  v-if="parseInt(item.orderStatus)>=0" class="item_wrap">
					...
					<div class="bottom">
						...
						<div class="order_look" @click.stop="lookDetail(index)" v-if="curType == 0">物流查询</div>
						<div class="btn_wrap">
							...
							<span class="pay" v-if="item.orderStatus == 1&&item.orderType==1&&item.isDaoFu==0" @tap="moneyPay(item.orderSN,item.orderID,item.price)">支付</span>
							<span class="pay" v-if="item.orderStatus==8" @click="moneyBack(index)">退款</span>
						</div>
