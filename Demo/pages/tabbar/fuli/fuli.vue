	...
	<view >
		<div class="fuli-top row_center" :style="{'padding-top': (isAPP?'30px':'0upx')}">
			...
			<div class="row_center head_wrap">
				<div class="col" @click="changeType(0)">
					<span>会员福利</span>
					<span :class="{'active': (curTypeIndex == 0)}"></span>
				</div>
				<div class="col" @click="changeType(1)">
					<span>线上特卖会</span>
		...
		<div  :style="{'margin-top': (isAPP?(curTypeIndex==0?'0upx':'200upx!important'):(curTypeIndex==0?'0upx':'140upx'))}">
			<navigator v-for="(item,index) in couponList" :key="index" @click="goToQuestion(item)"
				class="fuli_wrap" v-if="curTypeIndex == 0">
			...
			class="temai_wrap" v-if="curTypeIndex == 1">
				<img class="item_bg" :src="item.list_img" mode="aspectFill"/>
				<div class="row content_wrap">
...
<script>
		import { BASE_IMAGE_URL,sale_store,welfare_coupon_list,getBanners,welfare_coupon_time } from '@/utils/api'
	export default {
		data() {
			return {
				itemList: [],
		...
		methods: {
			goToQuestion(item){
				if(item.is_used == 1){
					//可以领取
					uni.navigateTo({
						url:'../../mall_question/mall_question?wc_id='+item.c_id
					})
				}else{
					wx.showToast({
						title: "目前不能领取此优惠券",
