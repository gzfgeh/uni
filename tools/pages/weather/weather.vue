<template>
	<view>
		...
		<image :src="weather_title" mode="aspectFill"  :style="{'top': tempHeight+'px'}"
			class="team_title" :animation="upDown"></image>
		
		<image :src="weather_bg" mode="aspectFill" class="team_bg"></image>	
		...
		<scroll-view class="content" scroll-y="true" @scroll="scroll" 
			...
			<view class="content_wrap" :style="{'height' : scrollHeightVal+'px'}" >
				<view v-for="(item,index) in list" :key="index" class="item" @click="clickItem(index)">
					<!-- <image :src="weather_item" mode="widthFix"></image>
					<span class="text">sssssssssssssssssswosssssssssssssssssswosssssssssssssssssswosssssssssssssssssswo</span>
					 -->
	...
	export default {
		data() {
			return {
				...
				weather_bg: BASE_IMAGE_URL+"weather_bg.png",
				white_back_icon: BASE_IMAGE_URL+"white_back_icon.png",
				weather_title: BASE_IMAGE_URL+"weather_title.png",
				weather_item: BASE_IMAGE_URL+"weather_item.png",
				animation: {},
				upDown: {},
				list: [],
				oldScroll: 0,
		...
		methods: {
			...
			async getContentList(){
				let res = await getContentList(4);
				if(res.code == 1000){
					this.list = res.data;
					this.list.map((item) => {
...
<style>
	...
	.team_title{ left: 34upx; position: absolute;z-index: 2; height: 108upx; width: 486upx;}
