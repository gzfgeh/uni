<template>
	<view>
		...
		<image :src="hotel_title" mode="aspectFill"  :style="{'top': tempHeight+'px'}"
			class="team_title" :animation="upDown"></image>
		
		<image :src="hotel_bg" mode="aspectFill" class="team_bg"></image>
		...
		<scroll-view class="content" scroll-y="true" @scroll="scroll" 
			...
			<view class="content_wrap" :style="{'height' : scrollHeightVal+'px'}" >
				<view v-for="(item,index) in list" :key="index" class="item" @click="clickItem(index)">
					<!-- <image :src="hotel_item" mode="widthFix"></image>
	...
	export default {
		data() {
			return {
				...
				hotel_bg: BASE_IMAGE_URL+"hotel_bg.png",
				white_back_icon: BASE_IMAGE_URL+"white_back_icon.png",
				hotel_title: BASE_IMAGE_URL+"hotel_title.png",
				hotel_item: BASE_IMAGE_URL+"hotel_item.png",
				animation: {},
				upDown: {},
				list: [],
				oldScroll: 0,
		...
		methods: {
			...
			async getContentList(){
				let res = await getContentList(2);
				if(res.code == 1000){
					this.list = res.data;
					this.list.map((item) => {
...
<style>
	...
	.team_title{ left: 34upx; position: absolute;z-index: 2; height: 108upx; width: 317upx;}
