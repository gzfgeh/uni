<template>
	<view>
		...
		<div class="no_style" v-if="isVip">卡号:{{member_no}}</div>
		
		
		<block v-for="(item,index) in list" :key="index">
			<navigator class="item_wrap" :url="'../vip_coupon_detail/vip_coupon_detail?id='+item.id">
...
<script>
	...
	export default {
		...
		onShow() {
			...
			if(userInfo.is_member == '1'){
				this.isVip = true;
				this.imgSrc = '../../static/img/my_bg.png';
			}else{
				this.isVip = false;
				this.imgSrc = '../../static/img/normal_bg.png';
			}
		}
...
.no_style{ position: absolute; top: 70upx; right: 10%; border-radius: 24upx; z-index: 2; font-size: 30upx; color: #FFF;}
