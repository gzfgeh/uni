...
function drawShareImage(obj) { //绘制海报方法
	...
	return new Promise((rs, rj) => {
		...
		console.log('准备执行draw方法')
		Context.draw(typeof(reserve) == 'boolean' ? reserve : false, () => {
			console.log('进入draw方法回调: ' + JSON.stringify(bgObj));
			let data = {
				x: 0,
				y: 0,
				width: bgObj.width,
...
function getShreUserPosterBackgroundFc(objs, upimage) { //下载并保存背景图方法
	...
	return new Promise((resolve, reject) => {
		...
		if (upimage) {
			...
			console.log(upimage);
			_app.getImageInfo_PromiseFc(upimage).then(imageObj => {
						let returnObj = {
							path: upimage,
							width: imageObj.width,
							height: imageObj.height,
			...
			_app.getPosterUrl(objs).then(image => {
				...
				_app.getImageInfo_PromiseFc(image).then(imageObj => {
							let returnObj = {
								path: image,
								width: imageObj.width,
								height: imageObj.height,
